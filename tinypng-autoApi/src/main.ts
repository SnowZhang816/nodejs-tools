...
class Main {
	...
	async getKeysFromCache(needNum: number): Promise<string[]> {
		return new Promise((resolve) => {
			const keyCachePath = path.join(__dirname, `keys.json`);
	...
	async startCompress(
		...
		while (failRetry--) {
			if (retryList.length) {
				logger.info(`存在重试列表,数量为 ${result.max - result.count} ,当前为第${this.conf.failRetry - failRetry}次重试`);
				let retryListNext: IFileCompressInfo[] = [];
				await this.createTaskQueue(retryList, result, retryListNext);
				retryList = retryListNext;
				logger.info(`重试压缩完成，已完成数量：${result.count},总数量:${result.max},总进度:${result.progress}%`);
			}
		}
	 ...
	 */
	async createTaskQueue(newList: IFileCompressInfo[], result: { count: number; progress: string; max: number }, retryList: IFileCompressInfo[]) {
		return new Promise((resolve) => {
